 ...
 * Из значений ГИС вычитается определенное значение
 * (минимальное, максимальное, средниее или медиана). Такая необходимость
 * может возникнуть при выводе данных скважинной магниторазведки,
 * поскольку при постановке этого метода не учитывались вариации
 * магнитного поля.
 * @author NovopashinAV
 *
 */

public class MeanGISStratigraphicLayerMinusValue extends Task
        implements nigp.tasks.MeanGIS, Stratigraphy, Micromine {
    ...
    private final String stratIntervals = "Интервалы искомых пластов";
    ...
    private String typeOfMinusValue = "отнять минимальное значение"; //выставлен по умолчанию

    public MeanGISStratigraphicLayerMinusValue(String[] inputParameters) throws TaskException {
    ...
    protected void processForCurrentExcel(SheetsOfExcelFile excelSheets)
        ...
        jointTable.forEach(e -> inputIntervalsForFindRocks(e, findStratigraphicIndexes,
        ...
        jointTable.forEach(e -> getMeanGISforIntervals(e, stratIntervals, nameMethodGIS));
        ...
        jointTable.forEach(e -> {
            int i = 0;
            if (e.get("Интервалы искомых пластов") != null) {
                String[] s = e.get("Интервалы искомых пластов").split("/");
                i = s.length;
            }
            e.put("Количество пластов", String.valueOf(i));
        });
