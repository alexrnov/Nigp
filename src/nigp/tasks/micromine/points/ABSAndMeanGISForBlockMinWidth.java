 ...
 * стратиграфический индекс определенного типа или равна ему.
 ...
 * запятую.
 * Экземпляр данного класса решает конкретную задачу. - Берутся скважины, по которым есть
 * хотя бы информация по четвертичке и юрским(без дяхтара) отложежениям и по ордовику. Если нет
 * информации по дяхтару и/или коре, программа добавляет эти пласты, и присваивает им
 * минимальную мощность. Это позволяет "не потерять" точки, по которым имеется типичный
 * (но неполный) стратиграфический разрез.
 */
public class ABSAndMeanGISForBlockMinWidth extends Task implements TablesAction,
    ...
    */
    ...
    public ABSAndMeanGISForBlockMinWidth(String[] inputParameters) throws TaskException {
    ...
    private void processForCurrentExcel(SheetsOfExcelFile excelSheets)
        ...
        */
        ...
        /*
        validPoints.forEach(e -> {
            System.out.println(e.get("Объект") + " " + e.get("Участок") + " "
                    + e.get("ID ТН") + " " + e.get("Стратиграфия"));
        });
        */

        validPoints.forEach(e -> transformSomeWellsToCorrectStructureConcrete(e));
        ...
        pointsWithTypeStructure.forEach(e -> {
            System.out.println(e.get("Стратиграфия"));
            System.out.println(e.get("Кровля стратопласта"));
            System.out.println(e.get("Подошва стратопласта"));
            System.out.println("---------------------");
        });
        */
        pointsWithTypeStructure.forEach(e -> establishMinWidth(e));
